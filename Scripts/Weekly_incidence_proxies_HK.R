...
# Load data
dat.fac <- readxl::read_excel(glue('{dat.dir}/HK/FLU-CL-AQ.xlsx'))

# Prepare plot data
UDFAB <- dat.fac %>% dplyr::select(KID = date, FLUAB) %>% mutate(KID = ymd(KID))
UDFA <- dat.fac %>% dplyr::select(KID = date, FLUA) %>% mutate(KID = ymd(KID))
UDFB <- dat.fac %>% dplyr::select(KID = date, FLUB) %>% mutate(KID = ymd(KID))
